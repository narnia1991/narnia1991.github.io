...
function App() {
    ...
    const shuffledPallete = Object.values(lightTheme).slice(1).map(value => ({ value, sort: Math.random() }))
        .sort((a, b) => a.sort - b.sort)
        .map(({ value }) => value)

    return (
        <div className="flex flex-col m-auto content-center flex-wrap lg:max-h-screen max-w-full">
            <NameCard />
            <WorkExperienceCard />
            {
